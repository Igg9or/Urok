...
Можно использовать: + - * / ^ sqrt() abs()`);
    ...
    async function saveAllTasks() {
        console.log("Подготовка к сохранению...");
        ...
        document.querySelectorAll('.task-card').forEach(taskElement => {
            tasksToSave.push({
                ...
                question: taskElement.querySelector('.task-text').value,
                answer_formula: taskElement.querySelector('.answer-formula').value,
                params: getTaskParams(taskElement)  // Ваша функция для сбора параметров
            });
        });
    
        console.log("Отправляемые данные:", tasksToSave);
        ...
        try {
            ...
            if (result.success) {
                alert(`Сохранено ${result.message}`);
                return true;
            } else {
