document.addEventListener('DOMContentLoaded', function() {
    ...
    async function generateStudentPreview(taskCard) {
        try {
            const templateId = taskCard.dataset.templateId;
            if (!templateId || !templatesCache[templateId]) {
                console.error('Template not found in cache');
                return;
            }

            const template = templatesCache[templateId];
            const question = taskCard.querySelector('.task-question').value;
            const answerTemplate = taskCard.querySelector('.task-answer').value;

            // Получаем параметры из сохраненного варианта или генерируем новые
            let params = {};
            const variantData = taskCard.dataset.variantData;
            if (variantData) {
                try {
                    params = JSON.parse(variantData).params;
                } catch (e) {
            ...
            if (Object.keys(params).length === 0) {
                const paramMatches = [...new Set([
                    ...(question.match(/\{([A-Za-z]+)\}/g) || []),
                    ...(answerTemplate.match(/\{([A-Za-z]+)\}/g) || [])
                ])].map(match => match.replace(/\{|\}/g, ''));

                paramMatches.forEach(param => {
                    params[param] = Math.floor(Math.random() * 10) + 1; // Генерация 1-10
                });

                // Если есть условия, выполняем их
                if (template.conditions) {
                    try {
                        const conditions = template.conditions.split(';');
                        conditions.forEach(cond => {
                            const [left, right] = cond.split('=').map(s => s.trim());
                            if (left && right) {
                                // Заменяем параметры в правой части
                                let expr = right;
                                for (const param in params) {
                                    expr = expr.replace(new RegExp(param, 'g'), params[param]);
                                }
                                params[left] = eval(expr);
                            }
                        });
                    } catch (e) {
                        console.error('Error executing conditions:', e);
                    }
                }
            }

            // Формируем вопрос для ученика
            let studentQuestion = question;
            for (const param in params) {
                studentQuestion = studentQuestion.replace(new RegExp(`\\{${param}\\}`, 'g'), params[param]);
            }
            ...
            if (previewQuestion) {
                previewQuestion.textContent = studentQuestion;
            }
            
            if (previewAnswer) {
                previewAnswer.textContent = result.success 
                    ? result.answer 
                    : "Ошибка в шаблоне ответа";
            }

            // Сохраняем сгенерированные параметры для повторного использования
            taskCard.dataset.variantData = JSON.stringify({
                params: params,
            ...
            console.error('Error in generateStudentPreview:', error);
        }
    }
