...
document.getElementById('analyzeTaskBtn').addEventListener('click', async function() {
    const analyzeBtn = this;
    const resultDiv = document.getElementById('aiResult');
    
    // Показываем индикатор загрузки
    analyzeBtn.disabled = true;
    analyzeBtn.innerHTML = '<span class="spinner"></span> Анализ...';
    ...
    try {
        ...
        analyzeBtn.textContent = 'Анализировать задание';
    }
});
...
function displayAiResult(result) {
    ...
    resultDiv.innerHTML = `
        <div class="ai-success">
            ...
            <p><strong>Шаблон:</strong> ${result.template}</p>
            <p><strong>Формула ответа:</strong> ${result.answer}</p>
            ${result.warnings ? `<div class="ai-warnings">Внимание: ${result.warnings.join(', ')}</div>` : ''}
            <button id="applyAiResult" class="btn btn-primary">Применить шаблон</button>
        </div>
    `;
    
    document.getElementById('applyAiResult').addEventListener('click', () => {
        applyTemplateToForm(result);
        resultDiv.classList.add('hidden');
    });
...
function showError(message) {
    ...
    resultDiv.innerHTML = `
        <div class="ai-error">
            ...
            <p>Попробуйте:</p>
            <ul>
                <li>Упростить формулировку</li>
                <li>Проверить изображение (если используется)</li>
                <li>Разделить сложную задачу на части</li>
            </ul>
        </div>
    `;
    resultDiv.classList.remove('hidden');
