document.addEventListener('DOMContentLoaded', function() {
    ...
    // Универсальная функция генерации ответа для любого шаблона
function computeAnswer(answerTemplate, params) {
    // 1. Подставляем значения параметров в шаблон
    let prepared = answerTemplate.replace(/\{([A-Za-z]+)\}/g, (_, p1) => params[p1]);

    // 2. Если выражение только числа и операции — вычисляем (это обычный пример)
    if (/^[\d\s\+\-\*\/\(\)\.]+$/.test(prepared)) {
        try {
            return eval(prepared).toString();
        } catch (e) {
            return "Ошибка в формуле ответа";
        }
    }
    // 3. Иначе (есть буквы — алгебра или строка) — просто возвращаем, не вычисляем!
    return prepared;
}
    ...
    try {
        ...
        // Вычисляем ответ
        variant.correct_answer = computeAnswer(template.answer_template, variant.params);
...
function randomInt(min, max) {
    ...
    function generateExample(questionTemplate, answerTemplate, taskCard) {
        ...
        // Вычисляем ответ
        exampleAnswer = computeAnswer(answerTemplate, params);
