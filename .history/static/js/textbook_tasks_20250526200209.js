...
document.getElementById('analyzeTaskBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        elements.templateName.value = `Сгенерированное задание ${new Date().toLocaleTimeString()}`;
        elements.questionTemplate.value = result.template;
        elements.answerTemplate.value = result.answer;
        
        // Создаем параметры
        elements.paramsContainer.innerHTML = '';
        for (const [param, config] of Object.entries(result.parameters)) {
            const group = document.createElement('div');
            group.className = 'param-group';
            group.innerHTML = `
                <h4>Параметр ${param}</h4>
                <div class="param-row">
                    <label>Тип: <input type="text" class="param-type" value="${config.type || 'int'}" readonly></label>
                    <label>Мин: <input type="number" class="param-min" value="${config.min}"></label>
                    <label>Макс: <input type="number" class="param-max" value="${config.max}"></label>
                </div>
                <div class="param-constraints">
                    ${(config.constraints || []).map(c => `
                        <div class="constraint">
                            <input type="text" class="constraint-value" value="${c}" readonly>
                        </div>
                    `).join('')}
                </div>
            `;
