async function saveEntireLesson() {
    const title = prompt('Введите название урока:', document.querySelector('.sidebar-header h2').textContent);
    if (!title) return;
    ...
    try {
        ...
        if (data.success) {
            ...
            const updatedResponse = await fetch(`/api/lessons/${lessonId}/tasks`);
...
// Добавим кнопку сохранения урока в интерфейс
function addSaveLessonButton() {
    const actionButtons = document.querySelector('.action-buttons');
    const saveLessonBtn = document.createElement('button');
    saveLessonBtn.className = 'btn btn-save-lesson';
    saveLessonBtn.textContent = 'Сохранить весь урок';
    saveLessonBtn.addEventListener('click', saveEntireLesson);
    actionButtons.appendChild(saveLessonBtn);
}
...
Можно использовать: + - * / ^ sqrt() abs()`);
    ...
    async function saveCurrentTask() {
        ...
        try {
            let response;
            if (currentTaskId.startsWith('new-')) {
                // Новое задание
            ...
            if (currentTaskId.startsWith('new-') && data.task_id) {
                // Обновляем ID для нового задания
                const taskIndex = tasks.findIndex(t => t.id === currentTaskId);
                if (taskIndex !== -1) {
                    tasks[taskIndex].id = data.task_id;
