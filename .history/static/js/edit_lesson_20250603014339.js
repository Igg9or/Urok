document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!templateId || !templatesCache[templateId]) return;

    const template = templatesCache[templateId];
    try {
        const params = {};
        // Заполнение параметров (существующий код)
        ...
        const response = await fetch('/api/generate_answer', {
            ...
            body: JSON.stringify({
                template: template.answer_template,
                params: params
            })
        });
        ...
        if (result.success) {
            // Обновление DOM
            taskCard.querySelector('.student-preview-question').textContent = 
                template.question_template.replace(/\{(\w+)\}/g, (_, p) => params[p]);
            taskCard.querySelector('.student-preview-answer').textContent = result.answer;
        }
    } catch (e) {
