document.addEventListener('DOMContentLoaded', function() {
    ...
    async function checkAnswer(taskCard) {
        ...
        let params = {};
            if (task.template_id) {
                // Загружаем параметры шаблона и генерируем с учетом условий
                const response = await fetch(`/api/templates/${task.template_id}`);
                const template = await response.json();
                params = MathEngine.generate_parameters(JSON.parse(template.parameters));
            } else {
                // Генерируем случайные значения
                params = generateRandomParams(task.question);
            }
