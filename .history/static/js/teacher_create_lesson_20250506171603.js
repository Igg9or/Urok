...
// В teacher_create_lesson.js
document.getElementById('generateTask').addEventListener('click', function() {
    const template = document.getElementById('taskEditor').value;
    const params = {};
    
    document.querySelectorAll('.param-group input, .param-group select').forEach(input => {
        const param = input.dataset.param;
        params[param] = {
            value: input.value,
            type: input.tagName === 'SELECT' ? 'choice' : 
                 input.step === '1' ? 'int' : 'float'
        };
    });
    
    fetch('/generate_task_variant', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            ...
            answer_formula: document.getElementById('answerFormula').value
        })
    })
    .then(response => response.json())
    .then(data => {
        ...
        addTaskToLesson(data.question, data.answer, data.params);
    })
    .catch(error => {
        console.error('Error:', error);
        alert('Ошибка генерации задания: ' + error.message);
    });
});