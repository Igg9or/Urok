...
document.addEventListener('DOMContentLoaded', function() {
        ...
        fetch(`/api/templates/${templateId}`)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Добавляем информацию о параметрах в HTML
                    const paramsInfo = generateParamsInfo(template.parameters, template.conditions);
                    addTask(template.question_template, template.answer_template, paramsInfo);
                }
            });
    }
    function generateParamsInfo(parameters, conditions) {
        ...
        for (const [param, config] of Object.entries(parameters)) {
            html += `<li><strong>${param}:</strong> ${config.min} ≤ ${param} ≤ ${config.max}`;
            if (config.constraints && config.constraints.length > 0) {
                ...
                for (const constraint of config.constraints) {
                    ...
                    // Добавьте другие типы ограничений по необходимости
                }
                html += '</ul>';
            }
            html += '</li>';
    ...
    function generateExample(questionTemplate, answerTemplate, paramsConfig = null) {
        ...
        if (paramsConfig) {
            for (const param in paramsConfig) {
                if (paramsConfig[param].type === 'int') {
                    let value;
                    let valid = false;
                    let attempts = 0;
                    ...
                    while (!valid && attempts < 100) {
                        value = getRandomInt(paramsConfig[param].min, paramsConfig[param].max);
                        valid = checkConstraints(value, paramsConfig[param].constraints);
                        attempts++;
                    }
                    
                    params[param] = value || paramsConfig[param].min; // fallback
                }
            }
        } else {
            // Генерируем простые значения (1-10) если нет конфигурации
    ...
    function checkConstraints(value, constraints) {
        ...
        for (const constraint of constraints) {
            ...
            if (constraint.type === 'greater_than' && value <= constraint.value) {
                return false;
            }
    ...
    if (paramsConfigElement) {
        try {
            // Можно добавить извлечение параметров из data-атрибутов или другим способом
            // В данном примере просто используем стандартную генерацию
        } catch (e) {
