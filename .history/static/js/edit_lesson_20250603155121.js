document.addEventListener('DOMContentLoaded', function() {
    ...
    function computeDerived(params, derived) {
    const result = {...params};
    if (!derived) return result;
    for (const key in derived) {
        try {
            // Элементарный безопасный eval только с известными переменными!
            result[key] = eval(derived[key].replace(/\b([A-Za-z_]\w*)\b/g, (m) => result[m]));
        } catch (e) {
            result[key] = 'err';
        }
    }
...
// Применять перед формированием строки ответа:
const paramsWithDerived = computeDerived(params, template.derived);
const answer = answerTemplate.replace(/\{(\w+)\}/g, (_, p1) => paramsWithDerived[p1]);
