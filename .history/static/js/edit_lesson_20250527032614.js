document.addEventListener('DOMContentLoaded', function() {
    ...
    async function generateExampleFromServer(question, answer, parameters) {
    try {
        const res = await fetch('/api/generate_task_instance', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ question, answer, parameters })
        });
        ...
        if (data.success) {
            ...
            console.error("Ошибка генерации примера:", data.error);
            return null;
        }
    } catch (e) {
...
function getParametersFromTaskCard(taskCard) {
    // Реализация извлечения параметров из формы (если есть специальные настройки)
    return {};
}
