    ...
    async function saveAllTasks() {
        try {
            const response = await fetch(`/api/save_tasks/${lessonId}`, {
                ...
                body: JSON.stringify({
                    tasks: tasks  // Отправляем все задания
                })
            });
            ...
            if (result.success) {
                ...
                // Обновляем ID заданий после сохранения
                if (result.updated_ids) {
                    tasks = tasks.map(task => {
                        const updated = result.updated_ids.find(t => t.temp_id === task.id);
                        return updated ? {...task, id: updated.new_id} : task;
                    });
                }
                return true;
            } else {
