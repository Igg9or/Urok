document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        // Загружаем как шаблоны из учебника, так и общие шаблоны уроков
        const [textbookTemplates, lessonTemplates] = await Promise.all([
            fetch(`/api/textbooks/${textbookId}/templates`).then(res => res.json()),
            fetch('/api/lesson_templates').then(res => res.json())
        ]);
        
        const allTemplates = [
            ...(textbookTemplates.success ? textbookTemplates.templates : []),
            ...(lessonTemplates.success ? lessonTemplates.templates : [])
        ];
