document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('saveAllBtn').addEventListener('click', async function() {
        try {
            ...
            // Сохраняем текущее задание (если редактировали)
            await saveCurrentTask();
            
            // Отправляем все задания на сервер
            ...
            if (result.success) {
                ...
                result.updatedTasks.forEach((serverId, index) => {
                    if (tasks[index].id.startsWith('new-')) {
                        tasks[index].id = serverId;
                    }
                });
                renderTaskList();
            }
            ...
            console.error('Ошибка:', error);
            alert('Не удалось сохранить урок: ' + error.message);
        } finally {
