document.addEventListener('DOMContentLoaded', function() {
    ...
    // БОЛЬШЕ НИЧЕГО! Не вызываем generateStudentPreview
}
    ...
    const previewElements = {
        question: taskCard.querySelector('.student-preview-question'),
        answer: taskCard.querySelector('.student-preview-answer')
    };

    // Установка состояния загрузки
    if (previewElements.question) previewElements.question.textContent = "Генерация...";
    if (previewElements.answer) previewElements.answer.textContent = "Генерация...";

    // Сначала получаем шаблон
    fetch(`/api/templates/${templateId}`)
        .then(res => res.ok ? res.json() : Promise.reject('Template load failed'))
        .then(templateData => {
            if (!templateData.success) throw new Error(templateData.error);
            
            // Затем генерируем вариант
            return fetch(`/api/generate_from_template/${templateId}`)
                .then(res => res.ok ? res.json() : Promise.reject('Generation failed'));
        })
        .then(data => {
            if (data.error) throw new Error(data.error);
            
            if (previewElements.question) {
                previewElements.question.innerHTML = data.question || "Не удалось сгенерировать вопрос";
            }
            if (previewElements.answer) {
                previewElements.answer.textContent = data.correct_answer || "Не удалось сгенерировать ответ";
            }
            
            if (window.MathJax) {
                MathJax.typesetPromise([previewElements.question]);
            }
        })
        .catch(error => {
            console.error('Preview generation error:', error);
            if (previewElements.question) previewElements.question.textContent = "Ошибка генерации";
            if (previewElements.answer) previewElements.answer.textContent = "Попробуйте позже";
        });
}
...
function randomInt(min, max) {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('btn-show-preview')) {
            ...
            const studentPreview = taskCard.querySelector('.student-preview');
            const isHidden = studentPreview.classList.contains('hidden');

            if (isHidden) {
                generateStudentPreview(taskCard); // Генерируем только сейчас!
