document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        ...
        // Вычисляем ответ
        try {
            // Если ответ - просто текст с параметрами (не математическое выражение!)
            // Если в шаблоне нет математических операторов, просто форматируем строку
            if (/^[^{}0-9+\-*/().]*$/.test(template.answer_template.replace(/\{([A-Za-z]+)\}/g, ''))) {
                // Простой текстовый ответ
                variant.correct_answer = template.answer_template.replace(
                    /\{([A-Za-z]+)\}/g, 
                    (_, p1) => variant.params[p1]
                );
            } else {
                // Обычный режим: вычисляем выражение
