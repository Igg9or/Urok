document.addEventListener('DOMContentLoaded', function() {
    ...
    function initTaskEditor() {
        console.log("Инициализация редактора заданий...");
    ...
    function loadTaskEditor(task) {
        console.log("Загрузка задания в редактор:", task);
    ...
    function renderTaskList() {
        console.log("Отрисовка списка заданий...");
    ...
    function updateTaskCounter() {
        const counter = document.getElementById('currentTaskNum');
        if (counter) {
            counter.textContent = currentTaskIndex + 1;
        }
    }
...
Можно использовать: + - * / ^ sqrt() abs()`);
    ...
    function renderParamControls(params) {
        ...
        if (!container) {
            console.error("Контейнер параметров не найден");
            return;
        }
    ...
    function generateExamples() {
        ...
        const count = parseInt(document.getElementById('examplesCount').value) || 3;
    ...
    function renderExamples(examples) {
        ...
        if (!container) {
            console.error("Контейнер примеров не найден");
            return;
        }
    ...
    async function saveCurrentTask() {
        ...
        document.querySelectorAll('.param-card').forEach(card => {
            ...
            params[name] = {
                ...
                min: parseFloat(card.querySelector('.param-min').value) || 1,
                max: parseFloat(card.querySelector('.param-max').value) || 10,
                step: parseFloat(card.querySelector('.param-step')?.value) || 1
            };
        });
        ...
        try {
            const response = await fetch(`/api/lessons/${lessonId}/tasks`, {
                ...
                body: JSON.stringify({
                    ...
                    params: params,
                    id: currentTaskId.startsWith('new-') ? null : currentTaskId
                })
            });
            ...
            if (data.success) {
                if (data.taskId) {
                    // Обновляем ID если задание новое
    ...
    console.log("Скрипт edit_lesson.js полностью загружен");
    console.log("ID текущего урока:", lessonId);
});