document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        const response = await fetch('/api/generate_task_variant', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                question: question,
                answer: answer
                // Можно передать свои параметры, например:
                // parameters: { A: { min: 5, max: 15 }, B: { min: 2, max: 8 } }
            })
        });
        ...
        if (data.success) {
            ...
            previewContainer.innerHTML = `
                <div class="example">
                    <p><strong>Вопрос:</strong> ${variant.question}</p>
                    <p><strong>Ответ:</strong> ${variant.correct_answer}</p>
                    <p class="params">Параметры: ${JSON.stringify(variant.params)}</p>
                </div>
            `;
        } else {
