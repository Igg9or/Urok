...
def api_check_answer():
    try:
        ...
        # 1. Проверка для числовых ответов
        ...
        # 2. Проверка для строковых ответов с вычислениями (типа "3 дм 5 см")
        if any(op in correct_answer for op in ['+', '-', '*', '/', '//', '%']):
            try:
                # Вычисляем правильный ответ по шаблону
                computed = MathEngine.evaluate_expression(correct_answer, params)
                if computed is not None:
                    return jsonify({
                        "is_correct": user_answer == computed,
                        "evaluated_answer": user_answer
                    })
            except:
                pass
        
        # 3. Простое строковое сравнение (для точных текстовых ответов)
        return jsonify({
