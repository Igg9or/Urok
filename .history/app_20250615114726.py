...
def api_check_answer():
    try:
        ...
        def clean_expression(expr):
            expr = expr.replace(" ", "")
            expr = expr.replace("^1", "")              # x^1 -> x
            expr = re.sub(r'([a-zA-Z])\^0', "1", expr) # x^0 -> 1
            expr = expr.replace("1*", "")              # 1*x -> x
        ...
        if answer_type == 'string':
            ...
            # Для сравнения знаков: > < =, убираем пробелы и сравниваем только значимые символыua = clean_expression(user_answer)
            ca = clean_expression(correct_answer)
