...
from math_engine import math_engine, ParameterGenerator
...
def api_generate_task():
    ...
    try:
        task = TaskGenerator.generate_from_template(template)
        if task:
            return jsonify(task)
        return jsonify({"error": "Failed to generate task"}), 400
    except Exception as e:
        return jsonify({"error": str(e)}), 500
