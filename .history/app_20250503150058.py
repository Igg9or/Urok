...
def update_task(task_id):
    ...
    try:
        ...
        ''', (data['question'], data['answer'], task_id, session['user_id']))
...
@app.route('/api/lessons/<int:lesson_id>/tasks', methods=['POST'])
def create_task(lesson_id):
    ...
    try:
        cursor.execute('''
            ...
            VALUES (?, ?, ?)
        ''', (lesson_id, data['question'], data['answer']))
...
@app.route('/api/lessons/<int:lesson_id>/tasks/batch', methods=['PUT'])
def update_lesson_tasks(lesson_id):
    ...
    try:
        # Получаем ID существующих заданий для сохранения
        keep_ids = [str(task['id']) for task in data['tasks'] if task.get('id') and not str(task['id']).startswith('new-')]
        
        # Удаляем задания, которые не были переданы (кроме сохраняемых)
        if keep_ids:
            ...
            # Если нет заданий для сохранения, удаляем все старые
            cursor.execute('''
                DELETE FROM lesson_tasks 
                WHERE lesson_id = ?
            ''', (lesson_id,))
        ...
        for task in data['tasks']:
            if not task.get('question') or not task.get('answer'):
                continue  # Пропускаем задания без текста или ответа
            
            if task.get('id') and not str(task['id']).startswith('new-'):
                # Обновление существующего задания
                cursor.execute('''
                    UPDATE lesson_tasks 
                    SET question = ?, answer = ?, params = ?
                    WHERE id = ? AND lesson_id = ?
                ''', (
                    task['question'],
                    task['answer'],
                ...
                # Добавление нового задания
                cursor.execute('''
                    INSERT INTO lesson_tasks (lesson_id, question, answer, params)
                    VALUES (?, ?, ?, ?)
                ''', (
                    lesson_id,
                    task['question'],
        ...
        return jsonify({
            'success': True,
            'updatedTasks': updated_ids,
            'message': f'Успешно обновлено {len(data["tasks"])} заданий'
        })
