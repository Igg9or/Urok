    ...
    try:
        ...
        if answer_type == 'string':
            ...
            def normalize_expr(expr):
                ...
                expr = re.sub(r'a\^0', '', expr)  # Удаляем a^0
                expr = re.sub(r'b\^0', '', expr)  # Удаляем b^0
                expr = re.sub(r'\*1', '', expr)    # Удаляем умножение на 1
                expr = re.sub(r'1\*', '', expr)    # Удаляем умножение на 1
                expr = re.sub(r'\+0', '', expr)    # Удаляем +0
                expr = re.sub(r'0\+', '', expr)    # Удаляем +0
                return expr
            # Для сравнения знаков: > < =, убираем пробелы и сравниваем только значимые символы
            ua = normalize_expr(user_answer)
        ca = normalize_expr(correct_answer)
