...
def check_answer():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT answer_template, answer_type FROM task_templates 
            WHERE id = (SELECT template_id FROM lesson_tasks WHERE id = ?)
        ''', (task_id,))
        template = cursor.fetchone()
        ...
        if template:
            ...
            if answer_type == 'text':
                # Для текстовых ответов сравниваем как есть
                is_correct = user_answer.strip() == correct_answer.strip()
            else:
                # Для числовых ответов вычисляем
                correct_val = MathEngine.evaluate_expression(correct_answer, {})
                user_val = MathEngine.evaluate_expression(user_answer, {})
