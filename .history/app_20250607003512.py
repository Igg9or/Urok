...
def get_template(template_id):
    ...
    try:
        template = conn.execute('''
            SELECT id, textbook_id, name, question_template, 
                   answer_template, parameters, answer_type,
                   COALESCE(derived_parameters, '{}') as derived_parameters
            FROM task_templates
            WHERE id = ?
        ...
        # Гарантированная обработка параметров
        try:
            template_dict['parameters'] = json.loads(template['parameters']) if template['parameters'] else {}
        except:
            template_dict['parameters'] = {}
...
def generate_from_template(template_id):
    ...
    try:
        ...
        # Обрабатываем параметры и derived_parameters
        try:
            template_dict['parameters'] = json.loads(template['parameters']) if template['parameters'] else {}
        except:
            template_dict['parameters'] = {}
        ...
        try:
            template_dict['derived_parameters'] = json.loads(template['derived_parameters']) if template.get('derived_parameters') else {}
        except:
            template_dict['derived_parameters'] = {}
        ...
        if not variant or 'question' not in variant:
