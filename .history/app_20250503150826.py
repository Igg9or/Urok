...
def update_lesson_tasks(lesson_id):
    ...
    try:
        ...
        # 1. Удаляем старые задания (кроме сохраняемых)
        existing_ids = [str(t['id']) for t in data['tasks'] if t.get('id') and not str(t['id']).startswith('new-')]
        if existing_ids:
            cursor.execute(f"""
                DELETE FROM lesson_tasks 
                WHERE lesson_id = ? 
                AND id NOT IN ({','.join(['?']*len(existing_ids))})
            """, [lesson_id] + existing_ids)
        else:
            cursor.execute("DELETE FROM lesson_tasks WHERE lesson_id = ?", (lesson_id,))

        # 2. Добавляем/обновляем задания
        for task in data['tasks']:
        ...
        return jsonify({
            'success': True,
            'updatedTasks': new_ids,
            'message': f'Сохранено {len(data["tasks"])} заданий'
        })
