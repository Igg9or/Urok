...
''')
    ...
    cursor.execute('''
        ...
        VALUES (1, 'Решите {A}x + {B} = {C}', '(C - B)/A', '{"A":{"type":"int","min":1,"max":10},"B":{"type":"int","min":5,"max":20},"C":{"type":"int","min":20,"max":50}}')
        ''')
    conn.commit()
    conn.close()
...
def save_lesson_task(lesson_id):
    print(f"Получены данные для сохранения: {request.json}")  # Логирование
    if 'user_id' not in session or session['role'] != 'teacher':
        return jsonify({'error': 'Unauthorized'}), 401
