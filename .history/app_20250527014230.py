...
def compute_answer(answer_template, params):
    try:
        expr = answer_template.format(**params)
        if ';' in expr:
            return '; '.join(str(eval(e.strip(), SAFE_GLOBALS, {})) for e in expr.split(';'))
        return str(eval(expr, SAFE_GLOBALS, {}))
    except Exception as e:
        return "Неверная формула ответа"
    
@app.route('/api/templates/<int:template_id>/example')
def get_template_example(template_id):
    ...
    try:
        ...
        if isinstance(parameters, dict):
            params = {
                k: random.randint(v['min'], v['max']) if isinstance(v, dict) else random.choice(v)
                for k, v in parameters.items()
            }
        elif isinstance(parameters, list):
            params = random.choice(parameters)
        else:
            return jsonify({'success': False, 'error': 'Неверный формат параметров'}), 400

        question = template['question_template'].format(**params)
        answer = compute_answer(template['answer_template'], params)
