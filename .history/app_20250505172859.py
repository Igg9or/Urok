...
def init_db():
    ...
    class_id INTEGER REFERENCES classes(id),  # Этот столбец должен быть
    title TEXT NOT NULL,
    date TEXT NOT NULL,
...
def create_lesson():
    ...
    # Получаем ID класса из формата "6В"
    grade = data['grade'][:-1]  # "6" из "6В"
    letter = data['grade'][-1]   # "В" из "6В"
...
def start_lesson(lesson_id):
    ...
    # Проверяем, имеет ли ученик доступ к этому уроку
    if session['role'] == 'student':
        cursor.execute('''
