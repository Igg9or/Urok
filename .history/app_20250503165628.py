...
def update_lesson(lesson_id):
    ...
    try:
        # Обновляем основную информацию об уроке
        cursor.execute('''
            UPDATE lessons 
            SET title = ?, date = ?
            WHERE id = ? AND teacher_id = ?
        ''', (data['title'], data['date'], lesson_id, session['user_id']))
        
        # Получаем текущие задания урока
        cursor.execute('SELECT id FROM lesson_tasks WHERE lesson_id = ?', (lesson_id,))
        existing_task_ids = [task['id'] for task in cursor.fetchall()]
        
        # Обновляем или добавляем задания
        for task in data['tasks']:
            if task.get('id') and task['id'] in existing_task_ids:
        ...
        # Удаляем задания, которых нет в новых данных
        new_task_ids = [task['id'] for task in data['tasks'] if task.get('id')]
        tasks_to_delete = set(existing_task_ids) - set(new_task_ids)
        
        for task_id in tasks_to_delete:
            cursor.execute('DELETE FROM lesson_tasks WHERE id = ?', (task_id,))
