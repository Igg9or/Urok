...
def api_check_answer():
    try:
        ...
        def normalize(s):
            # Убирает все пробелы и приводит к нижнему регистру
            return ''.join(str(s).lower().split())
        
        def is_percent_equivalent(user, correct):
            # Убрать %, пробелы и привести к float
            def to_number(s):
        ...
        if answer_type == 'algebraic':
            # (оставь свой код для алгебраических)
            pass
        ...
        try:
            ...
            # Здесь сравнение идёт ТАКЖЕ через normalize
            is_correct = normalize(user_answer) == normalize(correct_answer)
