...
def start_lesson(lesson_id):
    ...
    try:
        ...
        for task in base_tasks:
            ...
            if variant:
                ...
                try:
    # Проверяем, что параметры удовлетворяют условиям
    if template and template.get('conditions'):
        if not eval(template['conditions'], {}, params):
            raise ValueError("Параметры не удовлетворяют условиям шаблона")
    ...
    continue  # Пропускаем это задание и пробуем снова
                # Сохраняем вариант
                variant_data = {
                    'params': params,
