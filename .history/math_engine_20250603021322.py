...
def evaluate_expression(expr, params):
    try:
        # Если это строка с вычисляемыми выражениями в фигурных скобках
        if isinstance(expr, str) and re.search(r'\{[^}]+\}', expr):
            # Заменяем все {выражения} на их вычисленные значения
