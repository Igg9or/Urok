...
def api_check_answer():
    try:
        ...
        def normalize_fraction_string(expr_str):
            expr_str = expr_str.replace('^', '**')
            if '/' in expr_str:
                num, denom = expr_str.split('/', 1)
            else:
                num, denom = expr_str, "1"
            try:
                num_sym = sympify(num)
                denom_sym = sympify(denom)
                return num_sym / denom_sym
            except Exception:
                return None
        ...
        if answer_type == 'string':
            ...
            # Если оба ответа из одного символа, сравниваем как раньше
            if len(ua) == 1 and len(ca) == 1:
                is_correct = ua == ca
            # Если оба выглядят как дробь с буквами — пробуем сравнить как символьные дроби!
            elif re.match(r'^[\dA-Za-z\^\*]+/[A-Za-z\d\^\*]+$', ua) and re.match(r'^[\dA-Za-z\^\*]+/[A-Za-z\d\^\*]+$', ca):
                try:
                    ua_expr = normalize_fraction_string(ua)
                    ca_expr = normalize_fraction_string(ca)
                    if ua_expr is not None and ca_expr is not None:
                        is_correct = simplify(ua_expr - ca_expr) == 0
                    else:
                        is_correct = ua.lower() == ca.lower()
                except Exception:
                    is_correct = ua.lower() == ca.lower()
            # Если просто буквы, сравниваем без учета регистра и пробелов (старое поведение)
            else:
                is_correct = ua.lower() == ca.lower()
