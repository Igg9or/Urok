...
def save_lesson_task(lesson_id):
    print(f"Получены данные для сохранения: {request.json}")  # Для отладки
    if 'user_id' not in session or session['role'] != 'teacher':
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    try:
        # Проверяем принадлежность урока учителю
        cursor.execute('SELECT 1 FROM lessons WHERE id = ? AND teacher_id = ?', 
                      (lesson_id, session['user_id']))
        if not cursor.fetchone():
            return jsonify({'error': 'Lesson not found or access denied'}), 404
        ...
        if data.get('id'):
            ...
            cursor.execute('''
                ...
                SET question = ?, answer_formula = ?, params = ?
                WHERE id = ? AND lesson_id = ?
            ''', (question, answer_formula, params, data['id'], lesson_id))
            task_id = data['id']
        else:
        ...
        conn.rollback()
        print(f"Ошибка при сохранении: {str(e)}")  # Для отладки
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
        conn.close()
