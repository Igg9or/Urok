...
@app.route('/save_lesson_with_variants', methods=['POST'])
def save_lesson_with_variants():
    # ... получение данных урока ...
    
    for student in students:
        for task in tasks:
            # Генерация уникального варианта
            variant = generate_task_variant(task['template'], task['params_config'])
            
            # Сохранение варианта для ученика
            cursor.execute('''
                INSERT INTO student_task_variants
                (lesson_id, user_id, task_id, variant_data)
                VALUES (?, ?, ?, ?)
