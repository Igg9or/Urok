...
def start_lesson(lesson_id):
    ...
    try:
        ...
        for task in base_tasks:
            ...
            if variant:
                # Используем уже сохраненный вариант
                try:
                ...
                if task['template_id']:
                    ...
                    if template_row:
                        ...
                        try:
                            template_dict['parameters'] = json.loads(template_row['parameters']) if template_row['parameters'] else {}
                        except json.JSONDecodeError:
                            template_dict['parameters'] = {}
                        
                        try:
                            template_dict['derived_parameters'] = json.loads(template_row['derived_parameters']) if template_row.get('derived_parameters') else {}
                        except (json.JSONDecodeError, AttributeError):
                            template_dict['derived_parameters'] = {}
...
def api_check_answer():
    ...
    answer_type = data.get('answer_type', 'numeric')  # По умолчанию числовой
    
    try:
        # Поддержка старого формата (множественные ответы через ;)
        if ';' in correct_answer:
            correct_answers = [c.strip() for c in correct_answer.split(';')]
        else:
            correct_answers = [correct_answer]

        # Проверка для разных типов ответов
        def check_single_answer(user_ans, correct_ans):
            if answer_type == 'fraction':
                ...
                try:
                    expr1 = parse_expr(user_ans, transformations=standard_transformations)
                    expr2 = parse_expr(correct_ans, transformations=standard_transformations)
                    return sympy.simplify(expr1 - expr2) == 0
                except:
                    return False
            else:  # numeric (старый формат)
                try:
                    return abs(float(user_ans) - float(correct_ans)) < 1e-6
                except:
                    return False

        # Проверяем все варианты
        user_answers = [u.strip() for u in user_answer.split(';')] if ';' in user_answer else [user_answer]
        
        for u in user_answers:
            for c in correct_answers:
                if check_single_answer(u, c):
                    return jsonify({
                        'is_correct': True,
                        'correct_answer': correct_answer,
                        'matched_variant': f'Ваш ответ "{u}" совпал с "{c}"'
                    })
...
def generate_from_template(template_id):
    ...
    try:
        ...
        try:
            raw_derived = template['derived_parameters'] if 'derived_parameters' in template.keys() else None

            if raw_derived:
                template_dict['derived_parameters'] = json.loads(raw_derived)
            else:
                template_dict['derived_parameters'] = {}
        except Exception as e:
            print(f"[Ошибка чтения derived_parameters]: {e}")
            template_dict['derived_parameters'] = {}
        variant = TaskGenerator.generate_task_variant(template_dict)
