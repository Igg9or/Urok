...
class TaskGenerator:
    ...
    def generate_task_variant(template):
        ...
        # Вычисление derived-параметров
        if 'derived' in template:
            for key, expr in template['derived'].items():
                try:
                    # Преобразуем выражение, подставляя параметры
                    params[key] = int(sympy.sympify(expr, locals=params))
                except Exception as e:
                    print(f"Ошибка вычисления {key}: {expr} ({e})")
        ...
        # Вычисление ответа
        try:
            # Если в шаблоне используются только параметры — просто подставляем строку!
