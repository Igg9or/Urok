    ...
    try:
        ...
        for task in base_tasks:
            ...
            if variant:
                ...
                if task['template_id']:
                    cursor.execute('SELECT parameters, conditions FROM task_templates WHERE id = ?', 
                                 (task['template_id'],))
                    ...
                    if template:
                        ...
                        if template.get('conditions'):
                            try:
                                # Выполняем условия в контексте параметров
                                exec(template['conditions'], globals(), params)
                            except Exception as e:
                                print(f"Error processing template conditions: {e}")
                                # В случае ошибки - генерируем параметры заново
                                params = MathEngine.generate_parameters(template_params)
                ...
                # Вычисляем ответ (с обработкой строковых и числовых ответов)
                try:
                    # Пробуем вычислить числовой ответ
                    computed_answer = str(eval(answer_template.format(**params)))
                except:
                    # Если не получается - используем как строковый шаблон
