...
def bulk_delete_templates():
    ...
    conn = get_db()
    try:
        # Удаляем выбранные шаблоны
        placeholders = ','.join(['?'] * len(template_ids))
        query = f"DELETE FROM task_templates WHERE id IN ({placeholders}) AND textbook_id = ?"
        conn.execute(query, (*template_ids, textbook_id))
        conn.commit()
        return jsonify({
            'success': True,
            'deleted_count': len(template_ids)
        })
    except Exception as e:
        conn.rollback()
        return jsonify({'error': str(e)}), 500
