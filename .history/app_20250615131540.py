...
def api_check_answer():
    try:
        ...
        try:
            ...
            # Убираем * и возвращаем красивый текст
            return str(simplified).replace("**", "^").replace("*", "")
        except Exception:
            # Fallback на ручную очистку
            expr = expr.replace(" ", "")
            expr = expr.replace("^1", "")
            expr = re.sub(r'([a-zA-Z])\^0', "1", expr)
            expr = expr.replace("1*", "")
            expr = re.sub(r'\b1([a-zA-Z])', r'\1', expr)
            return expr
        ...
        if answer_type == 'string':
            ua = clean_expression(user_answer)
