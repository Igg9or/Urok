...
def generate_task_examples():
    ...
    try:
        ...
        examples = [TaskGenerator.generate_task_variant(question, answer, parameters) for _ in range(3)]
...
@app.route('/api/verify_answer', methods=['POST'])  # ← уникальное имя
def verify_answer():
    data = request.get_json()
    task_id = data['task_id']
    user_answer = data['user_answer']
...
@app.route('/api/generate_task_instance', methods=['POST'])
def generate_task_instance():
    from utils.task_generator import TaskGenerator  # если не импортирован
    data = request.get_json()
    ...
    try:
        task = TaskGenerator.generate_task_variant(question, answer, parameters)
        return jsonify({'success': True, 'instance': task})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 400
