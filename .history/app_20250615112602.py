...
from math_engine import compare_symbolic_or_numeric
...
def api_check_answer():
    try:
        ...
        if answer_type == 'string':
            ...
            # Если это математическое выражение — проверяем через символьное сравнение
            if any(c in ua + ca for c in "^*/+-=xyzabc()"):
                try:
                    is_correct = compare_symbolic_or_numeric(ua, ca)
                except Exception as e:
                    print("Sympy compare failed:", e)
                    is_correct = False
            else:
                # Просто текстовое сравнение
                is_correct = ua.lower().replace(" ", "") == ca.lower().replace(" ", "")
